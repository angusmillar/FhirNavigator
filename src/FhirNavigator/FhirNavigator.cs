...
public class FhirNavigator(
    ...
    FhirRepositorySettings fhirRepositorySettings) : IFhirNavigator
{
    public FhirRepositorySettings RepositorySettings => fhirRepositorySettings;
    ...
    public async Task<SearchInfo> Search<T>(SearchParams searchParams, int? pageLimiter = null) where T : Resource
    {
         return await fhirCallService.Search<T>(
            repositoryCode: fhirRepositorySettings.Code, 
    ...
    {
        T? fhirResource = await fhirCallService.Update(
            repositoryCode: fhirRepositorySettings.Code, 
            resource: resource, 
        ...
        {
            throw new ApplicationException($"Could not update {typeof(T).Name} FHIR resource for reference : " +
                                           $"{GetResourceTypeName<T>()}/{resource.Id}");
        }
    ...
    {
        T? fhirResource = await fhirCallService.Create(repositoryCode: fhirRepositorySettings.Code, resource: resource);
        if (fhirResource is null)
        {
            throw new ApplicationException($"Could not create {typeof(T).Name} FHIR resource for reference : " +
                                           $"{GetResourceTypeName<T>()}/{resource.Id}");
        }
    ...
    {
        await fhirCallService.Delete<T>(repositoryCode: fhirRepositorySettings.Code, resourceId: resourceId);
    }

    public async Task<Bundle> Transaction(Bundle transactionBundle)
    {
        return await fhirCallService.Transaction(repositoryCode: fhirRepositorySettings.Code, transactionBundle: transactionBundle);
    }
    ...
    public async Task<T?> GetResource<T>(string resourceId) where T : Resource
    {
        ...
        if (fhirResource is null)
        {
            fhirResource = await fhirCallService.GetById<T>(repositoryCode: fhirRepositorySettings.Code, resourceId);
        }
        ...
        Resource? parentResource = null) where T : Resource
    {
        ...
        FhirUri.FhirUri parsedResourceReference = fhirUriFactory.GetRequired(
            repositoryCode: fhirRepositorySettings.Code, 
            resourceReference: resourceReference.Reference, 
            errorLocationDisplay: errorLocationDisplay);

        if (string.IsNullOrWhiteSpace(parsedResourceReference.ResourceId))
        {
            throw new NullReferenceException($"Unable to locate resource id in the {nameof(resourceReference)} of " +
        ...
        if (parsedResourceReference.IsContained)
        {
            ...
            {
                throw new NullReferenceException($"The target Resource Reference is of type Contained however the parent " +
                                                 $"resource provided was null. Reference was: {resourceReference.Reference}. ");
            }

            if (parentResource is not DomainResource  parentDomainResource)
            {
                throw new ApplicationException(
                    $"The target Resource Reference is of type Contained however the parent resource has no is only a " +
                    $"DomainResource type which can no hold Contained resources. Reference was: {resourceReference.Reference}. ");
            }
            
            if (parentDomainResource.Contained is null)
            {
                throw new ApplicationException(
                    $"The target Resource Reference is of type Contained however the parent resource has no Contained " +
                    $"references. Reference was: {resourceReference.Reference}. ");
            }

            Resource? containedResource = parentDomainResource.Contained.FirstOrDefault(x => 
                x.Id.Equals(parsedResourceReference.ResourceId));
            ...
            {
                throw new ApplicationException(
                    $"The target Resource Reference is of type Contained however its matching Contained resource in of " +
                    $"type {containedResource.TypeName} and not the requested type of {typeof(T)}. Reference was: " +
        ...
        if (fhirResource is null)
        {
            return await fhirCallService.GetById<T>(
                repositoryCode: fhirRepositorySettings.Code, 
                parsedResourceReference.ResourceId);
        }
