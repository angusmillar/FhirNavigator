...
namespace FhirNavigator.Support;
...
public static class StringSupport
{
  ...
  public static string UrlPathCombine(string uri1, string uri2, char delimiter = '/')
  {
    uri1 = uri1.TrimEnd(delimiter);
    uri2 = uri2.TrimStart(delimiter);
    return $"{uri1}{delimiter}{uri2}";
  }
  
  public static string FormatHumanName(string family, IEnumerable<string>? givenList, IEnumerable<string>? titleList)
  {
    StringBuilder sb = new StringBuilder();

    string givenNames = string.Empty;
    if (givenList is not null && givenList.Any())
    {
      givenNames = string.Join(" ", givenList);
    }
    
    string titles = string.Empty;
    if (titleList is not null && titleList.Any())
    {
    ...
    {
      sb.Append(titles);
      if (!string.IsNullOrWhiteSpace(family) || string.IsNullOrWhiteSpace(givenNames))
      {
        sb.Append(' ');  
      }
    }
